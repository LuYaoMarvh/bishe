...
function displaySQL(sql) {
    document.getElementById('sqlCode').textContent = sql || '-- 未生成SQL';
}
    ...
    databases.forEach(db => {
        ...
        div.innerHTML = `
            ...
            <div class="db-list-item-meta">
                <div class="db-list-item-meta-item">📍 ${db.host}:${db.port}</div>
                <div class="db-list-item-meta-item">💾 ${db.name}</div>
            </div>
...
async function confirmSwitchDB() {
    ...
    try {
        ...
        document.querySelector('.loading-text').textContent = '正在切换数据库...';

        const response = await fetch('/api/databases/switch', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ database_id: currentSwitchDBId })
        });
        ...
        document.querySelector('.loading-text').textContent = '正在处理您的请求...';
        closeSwitchDB();
    }
}
...
async function submitAddDatabase(event) {
    ...
    // 验证必填字段
    if (!config.id || !config.name || !config.display_name || !config.host || !config.user) {
        alert('请填写所有必填字段（带*的字段）');
        return;
    }
...
async function deleteDatabase(dbId, dbName) {
    ...
    try {
        const response = await fetch(`/api/databases/${dbId}`, { method: 'DELETE' });
        const data = await response.json();
...
function openSettings() {
    ...
    document.querySelectorAll('.settings-tab-content').forEach((content, idx) => {
...
async function loadStats() {
    try {
        ...
        document.getElementById('stat-total').textContent = stats.total_queries || '0';
        document.getElementById('stat-success').textContent =
            (stats.success_rate !== undefined ? stats.success_rate + '%' : '-');
        document.getElementById('stat-clarify').textContent = stats.clarification_count || '0';
...
function setLoading(loading) {
    ...
    document.getElementById('sendBtnLoader').style.display = loading ? 'inline' : 'none';
}
...
window.onclick = function(event) {
    const modals = ['clarificationModal', 'settingsModal', 'addDatabaseModal', 'switchDBModal'];
